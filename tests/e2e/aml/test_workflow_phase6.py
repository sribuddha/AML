...
def mock_llm(monkeypatch):
    async def mock_triage(*args, **kwargs):
        return TriageDecision(escalate=True, reason="Phase6 escalation", confidence=0.9)

    async def mock_stage3(*args, **kwargs):
        return TriageDecision(escalate=True, reason="Phase6 deep-dive confirms", confidence=0.95)

    async def mock_sar(*args, **kwargs):
        return SarResult(content="Phase6 SAR report body", raw_response='{"sar": "phase6"}')
