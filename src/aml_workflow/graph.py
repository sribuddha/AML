...
def create_workflow(db: AsyncSession, llm: LLMClient | None = None, mode: str = "full", checkpointer=None):
    ...
    async def rule_engine_batch(state: WorkflowState) -> dict:
        ...
        logger.info("Rule engine (pre-velocity): %d flagged, %d clean out of %d", flagged_count, len(results) - flagged_count, len(results))
        ...
        for cid, txns in cust_txns.items():
            ...
            if velocity_zscore is not None and velocity_zscore > VELOCITY_ZSCORE_THRESHOLD:
                for t in txns:
                    setattr_rules.setdefault(t["id"], {})["velocity_zscore"] = (
                        f"Velocity Check (z-score: {velocity_zscore:.1f})"
                    )
            if structuring_count >= STRUCTURING_24H_THRESHOLD:
                for t in txns:
                    if t["date"] and datetime.fromisoformat(t["date"]) >= one_day_ago:
    ...
    async def stage2_triage(state: WorkflowState) -> dict:
        ...
        llm_batch: list[tuple[dict, dict, dict | None]] = []
    ...
    async def stage3_triage(state: WorkflowState) -> dict:
        ...
        if customer_ids and mode in ("stage3", "full"):
            ...
            for t in recent_rows:
                txn_id = str(t.id)
                cid = t.customer_id
                # Skip current transaction references per-customer per-escalated-item, handled per-result below
