...
- Name: layout.spec.ts >> Sidebar Layout >> sidebar shows all navigation items
- Location: ..\tests\e2e\ui\layout.spec.ts:4:7
...
Locator: getByText('Compliance')
Expected: visible
Error: strict mode violation: getByText('Compliance') resolved to 2 elements:
    1) <span>Compliance</span> aka getByRole('link', { name: '🛡️ Compliance 3' })
    2) <h3 class="text-base font-semibold text-slate-800 group-hover:text-blue-600 transition-colors">Compliance</h3> aka getByRole('link', { name: '🛡️ Compliance Review' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Compliance')
