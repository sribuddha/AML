# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: rules.spec.ts >> Rules Page >> shows status in edit form
- Location: ..\tests\e2e\ui\rules.spec.ts:64:7

# Error details

```
Error: expect(locator).toBeVisible() failed

Locator: locator('form select').nth(1)
Expected: visible
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('form select').nth(1)

```

```yaml
- complementary:
  - link "AML Monitor":
    - /url: /
  - navigation:
    - link "🛡️ Compliance 3":
      - /url: /compliance
    - button "📤 Operations ▼"
    - link "↑ Upload":
      - /url: /operations
    - link "⚙ Rules":
      - /url: /operations/rules
    - link "👥 Customers":
      - /url: /customers
    - link "📋 Transactions":
      - /url: /transactions
    - separator
    - link "🧪 Test Data Generator":
      - /url: /test
  - link "📄 API Docs":
    - /url: /docs
- main:
  - heading "Rules" [level=3]
  - paragraph: Manage AML detection rules
  - text: Type
  - textbox "Filter by type"
  - text: Status
  - combobox:
    - option "All" [selected]
    - option "Active"
    - option "Inactive"
  - text: Name
  - textbox "Filter by name"
  - button "Search"
  - button "+ Add Rule"
  - heading "Edit Rule" [level=4]
  - text: Name
  - textbox: High Value Check
  - text: Type
  - combobox:
    - option "deterministic" [selected]
  - text: Status
  - combobox:
    - option "active" [selected]
    - option "inactive"
  - text: Description
  - textbox: Flags transactions over $10,000
  - text: Rules JSON
  - textbox: "[ { \"field\": \"amount\", \"operator\": \">\", \"value\": 10000 } ]"
  - paragraph: JSON array of rule conditions/actions
  - button "Save"
  - button "Cancel"
  - table:
    - rowgroup:
      - row "Name Type Status Description":
        - columnheader "Name"
        - columnheader "Type"
        - columnheader "Status"
        - columnheader "Description"
        - columnheader
    - rowgroup:
      - row "High Value Check deterministic active Flags transactions over $10,000 Deactivate":
        - cell "High Value Check":
          - button "High Value Check"
        - cell "deterministic"
        - cell "active"
        - cell "Flags transactions over $10,000"
        - cell "Deactivate":
          - button "Deactivate"
      - row "Negative Amount deterministic active Flags negative transaction amounts Deactivate":
        - cell "Negative Amount":
          - button "Negative Amount"
        - cell "deterministic"
        - cell "active"
        - cell "Flags negative transaction amounts"
        - cell "Deactivate":
          - button "Deactivate"
      - row "Offshore Location deterministic active Flags transactions from offshore locations Deactivate":
        - cell "Offshore Location":
          - button "Offshore Location"
        - cell "deterministic"
        - cell "active"
        - cell "Flags transactions from offshore locations"
        - cell "Deactivate":
          - button "Deactivate"
      - row "High Risk Jurisdiction deterministic active Flags transactions to/from high-risk jurisdictions Deactivate":
        - cell "High Risk Jurisdiction":
          - button "High Risk Jurisdiction"
        - cell "deterministic"
        - cell "active"
        - cell "Flags transactions to/from high-risk jurisdictions"
        - cell "Deactivate":
          - button "Deactivate"
      - row "Offshore Counterparty deterministic active Flags transactions with offshore-related counterparties Deactivate":
        - cell "Offshore Counterparty":
          - button "Offshore Counterparty"
        - cell "deterministic"
        - cell "active"
        - cell "Flags transactions with offshore-related counterparties"
        - cell "Deactivate":
          - button "Deactivate"
      - row "Threshold Proximity deterministic active Flags transaction amounts near reporting threshold Deactivate":
        - cell "Threshold Proximity":
          - button "Threshold Proximity"
        - cell "deterministic"
        - cell "active"
        - cell "Flags transaction amounts near reporting threshold"
        - cell "Deactivate":
          - button "Deactivate"
      - row "Round Amount deterministic active Flags round-dollar amounts often used in structuring Deactivate":
        - cell "Round Amount":
          - button "Round Amount"
        - cell "deterministic"
        - cell "active"
        - cell "Flags round-dollar amounts often used in structuring"
        - cell "Deactivate":
          - button "Deactivate"
```

# Test source

```ts
  1  | import { test, expect } from "@playwright/test";
  2  | 
  3  | test.describe("Rules Page", () => {
  4  |   test.beforeEach(async ({ page }) => {
  5  |     await page.goto("/operations/rules");
  6  |     await page.waitForLoadState("networkidle");
  7  |   });
  8  | 
  9  |   test("renders heading and description", async ({ page }) => {
  10 |     await expect(page.getByRole("heading", { name: "Rules" })).toBeVisible();
  11 |     await expect(page.getByText("Manage AML detection rules")).toBeVisible();
  12 |   });
  13 | 
  14 |   test("shows seeded rules in table", async ({ page }) => {
  15 |     const rows = page.locator("table tbody tr");
  16 |     await expect(rows.first()).toBeVisible({ timeout: 10000 });
  17 |   });
  18 | 
  19 |   test("filters by status", async ({ page }) => {
  20 |     await page.selectOption("select", "inactive");
  21 |     await page.getByRole("button", { name: "Search" }).click();
  22 |     await page.waitForLoadState("networkidle");
  23 |     const badges = page.locator("table tbody tr td span.rounded-full");
  24 |     const count = await badges.count();
  25 |     for (let i = 0; i < count; i++) {
  26 |       await expect(badges.nth(i)).toHaveText("inactive");
  27 |     }
  28 |   });
  29 | 
  30 |   test("creates a new rule", async ({ page }) => {
  31 |     await page.getByRole("button", { name: "+ Add Rule" }).click();
  32 |     await page.fill("input", "Test E2E Rule");
  33 |     await page.locator("textarea").fill(JSON.stringify([{ field: "amount", operator: "gt", value: 10000 }], null, 2));
  34 |     await page.getByRole("button", { name: "Save" }).click();
  35 |     await page.waitForLoadState("networkidle");
  36 |     await expect(page.getByText("Test E2E Rule")).toBeVisible();
  37 |   });
  38 | 
  39 |   test("edits an existing rule", async ({ page }) => {
  40 |     const nameCell = page.locator("table tbody tr").first().locator("td").first().locator("button");
  41 |     await nameCell.click();
  42 |     await page.fill("input", "Edited E2E Rule");
  43 |     await page.getByRole("button", { name: "Save" }).click();
  44 |     await page.waitForLoadState("networkidle");
  45 |     await expect(page.getByText("Edited E2E Rule")).toBeVisible();
  46 |   });
  47 | 
  48 |   test("deactivates and reactivates a rule", async ({ page }) => {
  49 |     const toggleBtn = page.locator("table tbody tr").first().locator("td").last().locator("button");
  50 |     const initialText = await toggleBtn.textContent();
  51 |     const wasActive = initialText?.trim() === "Deactivate";
  52 | 
  53 |     await toggleBtn.click();
  54 |     page.on("dialog", (d) => d.accept());
  55 |     await page.waitForLoadState("networkidle");
  56 | 
  57 |     const badge = page.locator("table tbody tr").first().locator("td").nth(2).locator("span.rounded-full");
  58 |     await expect(badge).toHaveText(wasActive ? "inactive" : "active");
  59 | 
  60 |     const toggleBtnAgain = page.locator("table tbody tr").first().locator("td").last().locator("button");
  61 |     await expect(toggleBtnAgain).toHaveText(wasActive ? "Activate" : "Deactivate");
  62 |   });
  63 | 
  64 |   test("shows status in edit form", async ({ page }) => {
  65 |     const nameCell = page.locator("table tbody tr").first().locator("td").first().locator("button");
  66 |     await nameCell.click();
  67 |     const statusSelect = page.locator("form select").nth(1);
> 68 |     await expect(statusSelect).toBeVisible();
     |                                ^ Error: expect(locator).toBeVisible() failed
  69 |     await expect(statusSelect).toHaveValue(/active|inactive/);
  70 |   });
  71 | });
  72 | 
```