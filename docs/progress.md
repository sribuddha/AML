...
| **Backend — Workflow** | LLM call timeout (`AML_LLM_TIMEOUT`, per-call) | ✅ Done |
| | Per-upload LLM budget (`AML_LLM_BUDGET`, hard stop + fallback) | ✅ Done |
| | AML_Workflow — deterministic rule engine (LangGraph, 100-txn batches) | ✅ Done |
...
| **Eval Harness** | Confidence calibration (decile bins, ECE in eval reports) | ✅ Done |
| | Fraud pattern generator (5 patterns, ground truth labels) | ✅ Done |
...
| Alembic autogenerate safety | `src/bff/migrations/env.py` now imports all 11 model modules (not just `Base`), so `Base.metadata` is fully populated when alembic compares schema vs schema. Previously it imported only `Base` → empty metadata → autogenerate produced DROP TABLE for every table. |
...
- **Python CVE scan**: `pip-audit` added as dev dep via `uv add --group dev pip-audit bandit` — run with `uv run pip-audit`
- **Python SAST scan**: `bandit` added — run with `uv run bandit -r src/` (5357 lines scanned, 0 issues)
- **Frontend CVE scan**: `npm audit` in `ui/` — 0 vulnerabilities
- **Runbook entries**: All three scan commands documented in `docs/runbook.md`

### Langfuse Diagnostics & Robustness (✅ Done)
- **`.env` path fix**: `observability.py` now derives `.env` path from `Path(__file__).resolve()` instead of depending on `os.getcwd()` — eliminates silent auth failures when uvicorn CWD differs from project root
- **CallbackHandler logging**: All `except Exception` blocks in `_langfuse.py` now log the actual error via `logger.warning()` — no more silent fallbacks
- **Explicit credentials**: `CallbackHandler(public_key=self._public_key)` passes the key explicitly instead of relying solely on env vars

### Alembic Autogenerate Safety (✅ Done)
- **Root cause**: `env.py` imported `Base` but no model modules, so `Base.metadata` was empty → autogenerate compared 0 tables against 12 DB tables → generated DROP TABLE for everything
- **Fix**: `env.py` now imports all 11 model modules, fully populating `Base.metadata` before alembic comparison
- **AGENTS.md**: New "Alembic & Schema Safety" section with rules against dropping tables, requiring review of autogenerated migrations, and manual `add_column` preference
...
### Phase 2.1 — Data-Leak Warning + Anonymization (✅ Done)
- **Startup warning**: When `AML_ANONYMIZE_LLM_DATA` is `false` (default), logs a warning on app startup that PII is being sent to external LLM providers
- **`_mask_sensitive_fields()`**: Module-level function in `llm.py` — returns a copy of a transaction dict with `counterparty` replaced by `"[REDACTED]"`
- **`_build_txn_json_block()` updated**: Checks `get_anonymize_llm_data()` at call time; when `true`, masks counterparty before building the JSON block
- **All prompt paths covered**: triage, stage3, SAR, and all batch variants automatically get anonymization through `_build_txn_json_block`
- **Tests**: 7 new tests covering masking, round-trip via JSON block, batch items, no-masking when disabled

### Phase 2.2 — Pre-commit Hook + Key Rotation (✅ Done)
- **`.pre-commit-config.yaml`**: Added with `detect-private-key`, `mixed-line-ending`, `end-of-file-fixer`, and custom `scan-for-secrets` hook
- **`scripts/scan_for_secrets.py`**: Scans staged text files for OpenAI keys (`sk-...`), Gemini keys (`AIza...`), AWS keys, private key PEM blocks, and generic `api_key`/`secret`/`token` assignments
- **False-positive guard**: Skips env-var reference patterns (`VITE_AML_API_KEY`, `AML_*_API_KEY`, `LANGFUSE_*`)
- **`pre-commit` added to dev dependencies** in `pyproject.toml`
- **Runbook updated**: `pre-commit install` setup + manual scan command + key rotation procedure
...
### Phase 7 — Eval Harness + Improvements (✅ Done)
- Calibration check — decile-binned confidence vs accuracy analysis (`CalibrationBin` in eval responses, `compute_calibration()` in `src/aml_workflow/eval/calibration.py`)
...
### Phase 4.1 — Per-upload LLM Budget (✅ Done)
- `AML_LLM_BUDGET` env var (USD, 0 = unlimited) caps total LLM spend per upload
- `LLMClient` tracks running cost via `_estimate_call_cost()` — conservative model-aware pricing (gpt-4o-mini/gpt-4o/gemini-2.0-flash)
- Budget check before every LLM call; fallback to rule-based defaults when exceeded
- `_total_cost` exposed for observability

### Phase 4.3 — LLM Call Timeout (✅ Done)
- `AML_LLM_TIMEOUT` env var (seconds, default 120) per-call timeout
- `timeout` parameter passed to all OpenAI and Gemini SDK calls (triage, stage3, SAR, batch variants)
- Catches timeout exceptions via existing `APIError` handlers → fallback to defaults
...
### Ops Readiness — Health Endpoint (✅ Done)
- `GET /api/health` — DB connectivity probe + version, bypasses API key auth
- 3 tests covering success, DB error, and auth bypass

### Ops Readiness — Containerization (✅ Done)
- `Dockerfile`: Multi-stage (Node builds frontend → Python deps → slim runtime), 488MB image (`aml-app`)
- `Dockerfile.nginx`: Frontend build + nginx:alpine, 93.3MB image (`aml-nginx`)
- `nginx.conf`: Reverse proxy `/api/*` + `/docs` + `/openapi.json` to backend, SPA fallback
- `.dockerignore`: Build context exclusions
- `docker-compose.yml`: `app` + `nginx` services, `app-data` volume, nginx as entry point (port 80)
- Docker build fix: 40 TypeScript errors resolved — test files excluded in tsconfig, unused imports removed, type narrowing fixes

### Settings Page + Auth UX (✅ Done)
- `SettingsPage.tsx` — localStorage-based API key input, test-on-save validation, clear key
- `Layout.tsx` — bootstrap auth check (`/api/sar`) on mount detects 401 → locked nav (Settings + API Docs only) or full nav
- `client.ts` — `getApiKey()` reads localStorage first, falls back to env var, `setApiKey()`/`clearApiKey()` helpers
- `App.tsx` — `/settings` route added
- 11 SettingsPage tests + 6 locked-state Layout tests
- Runbook updated with Production Deployment section
...
- Authentication / authorization (multi-user, RBAC)
- CI/CD pipeline
- MCP enrichment via LangGraph tools
